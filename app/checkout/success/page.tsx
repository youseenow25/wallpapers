      ...
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-4px)] px-6 py-20">
        <div className="w-full max-w-lg">
          ...
          {order.downloads.length > 0 && (
            <div className="border border-[#ddd5c4] bg-white/30 rounded-sm divide-y divide-[#ddd5c4] mb-6 text-sm">
              <p className="px-6 py-3 text-[9px] uppercase tracking-[0.2em] text-[#a09880]">
                Your downloads
              </p>
              {order.downloads.map((d) => (
                <div key={d.url} className="flex items-center justify-between gap-4 px-6 py-3.5">
                  <span className="font-medium text-[#1c1a18] truncate">{d.title}</span>
                  <a
                    href={d.url}
          ...
          <div className="border border-[#ddd5c4] bg-white/30 rounded-sm divide-y divide-[#ddd5c4] mb-10 text-sm">
            ...
            {order.discountPercent > 0 && (
              <div className="flex justify-between px-6 py-3.5">
                ...
                <span className="font-medium text-green-700">{order.discountPercent}% off</span>
              </div>
            )}
          ...
          <p className="text-center text-xs text-[#a09880] mb-8 leading-relaxed">
            Downloads expire in 7 days. Trouble with a link?{" "}
